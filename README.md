...
運用画面を優先するため、設定系UIは常時表示せず、上部の `Mission Setup / YAML` ボタンから開くモーダルウィンドウに集約しています。
...
TLE URL貼り付け、3行TLE貼り付け、対象衛星選択、地上局選択、表示衛星選択、YAML Import/Export、表示系詳細設定、YAMLエディタは `Mission Setup / YAML` ウィンドウ内に置いています。設定の正本は引き続きYAMLです。
