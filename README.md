...
既存の `tle_pass_csv_exporter` と同じ運用思想に寄せ、TLE を YAML に直接書き、指定日の AOS〜LOS 区間をパスごとに分けて CSV/DEF ZIP 出力できます。
...
- 指定日1日分のパス別 Doppler CSV/DEF ZIP 出力
...
- `OperationalPreparationProcedures.md`: 運用準備手順。YAML一括設定、Visible Passes、Ops予約、Doppler CSV/DEF出力の手順を記載。
...
DEF出力の内部オフセットは、通常はYAMLへ書かず、標準値 `2158000000 Hz` を自動使用します。地上局側のDEF仕様が変わる場合だけ、互換用キー `doppler.def_frequency_offset_hz` で上書きできます。
